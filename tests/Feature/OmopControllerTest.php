...
class OmopControllerTest extends TestCase
{
    ...
    {
        $response = $this->getJson(self::SEARCH_URL . '?concept_name[]=sickle');
    ...
    {
        $response = $this->getJson(self::SEARCH_URL . '?concept_id[]=24006');
    ...
    {
        // concept_name matches sickle rows; concept_id matches hypertension row — all three returned
        $response = $this->getJson(self::SEARCH_URL . '?concept_name[]=sickle&concept_id[]=320128');
    ...
    {
        // concept_name[]=sickle matches 2 rows
        // concept_name[]=hypertension matches 1 row
        // concept_id[]=320128 also matches that hypertension row (deduped via DISTINCT)
        $response = $this->getJson(
            self::SEARCH_URL . '?concept_name[]=sickle&concept_name[]=hypertension&concept_id[]=320128'
        );
    ...
    {
        $response = $this->getJson(self::SEARCH_URL . '?domain=Measurement');
    ...
    {
        $response = $this->getJson(self::SEARCH_URL);
    ...
    {
        // The old ?description= param should not match anything (it's ignored now)
        $response = $this->getJson(self::SEARCH_URL . '?description=sickle');

        $response->assertOk();
        // All 4 rows returned because 'description' is not a recognised search key
        $this->assertEquals(4, $response->json('data.total'));
    }
}
