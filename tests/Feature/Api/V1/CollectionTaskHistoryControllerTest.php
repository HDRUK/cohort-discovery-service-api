...
class CollectionTaskHistoryControllerTest extends TestCase
{
    ...
    #[\PHPUnit\Framework\Attributes\Test]
    public function it_returns_a_null_query_for_a_task_whose_query_was_soft_deleted()
    {
        $task = $this->succeededTask(500);
        ...
        $this->assertSame((string) $task->pid, $item['pid']);
        $this->assertNull($item['query']);
    }

    #[\PHPUnit\Framework\Attributes\Test]
    public function it_omits_the_query_definition_by_default()
    {
        $this->succeededTask(500);

        $response = $this->actingAsJwt($this->adminUser)->getJson($this->url());

        $response->assertOk();
        $this->assertArrayNotHasKey('definition', $response->json('data.tasks.data.0.query'));
    }

    #[\PHPUnit\Framework\Attributes\Test]
    public function it_includes_the_query_definition_when_asked()
    {
        $this->query->update(['definition' => ['some' => 'rule tree']]);
        $this->succeededTask(500);

        $response = $this->actingAsJwt($this->adminUser)
            ->getJson($this->url(['include' => 'definition']));

        $response->assertOk();
        $this->assertSame(
            ['some' => 'rule tree'],
            $response->json('data.tasks.data.0.query.definition')
        );
    }

    #[\PHPUnit\Framework\Attributes\Test]
    public function it_rejects_an_unknown_include()
    {
        $this->actingAsJwt($this->adminUser)
            ->getJson($this->url(['include' => 'everything']))
            ->assertStatus(422)
            ->assertJsonValidationErrors('include');
    }

    #[\PHPUnit\Framework\Attributes\Test]
    public function it_returns_every_documented_field_for_a_task()
    {
        $this->succeededTask(1900);
            ...
            ->assertJsonStructure([
                'data' => [
                    ...
                    'summary' => [
                        ...
                        'pending',
                        'task_types' => ['a', 'b'],
                        'attempts' => ['total', 'retried_tasks', 'max'],
                        'duration_ms' => ['runs_measured', 'min', 'avg', 'p50', 'p95', 'max'],
                    ],
                    'tasks' => [
                        'current_page',
                        'per_page',
    ...
    #[\PHPUnit\Framework\Attributes\Test]
    public function it_forbids_before_it_complains_about_the_range()
    {
        $outsider = User::factory()->create();

        $this->actingAsJwt($outsider)
            ->getJson($this->url(['window' => 'not-a-window']))
            ->assertForbidden();
    }

    #[\PHPUnit\Framework\Attributes\Test]
    public function it_reports_not_found_before_it_complains_about_the_range()
    {
        $this->actingAsJwt($this->adminUser)
            ->getJson('/api/v1/collections/'.Str::uuid()->toString().'/task-history?window=not-a-window')
            ->assertNotFound();
    }
